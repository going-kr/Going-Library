...
| ↳ 컨트롤 JSON | `ui-json-controls.md` | GoButton, GoSlider 등 33종 주요 컨트롤 |
...
| ↳ 복잡한 컨트롤 C# 코드 | `ui-control-sample.md` | GoBarGraph, GoDataGrid, GoListBox 등 Series/Items 설정이 필요한 컨트롤의 C# 바인딩 예제. **로드 조건: C# 코드에서 그래프·그리드·리스트 등 데이터 바인딩이 필요한 컨트롤을 다룰 때만 읽을 것** |
...
| 작업 | 1순위 | 2순위 | 3순위 (필요 시) |
|------|-------|-------|----------------|
| .gud 파일 생성 | `ui-json.md` | `ui-json-controls.md` | `ui-json-input-value.md` (GoInput/GoValue) / `ui-json-containers.md` / `ui-json-imagecanvas.md` |
...
```
// GoButton 속성 확인
Read("__SKILL_DIR__/api/ui/controls.html")

// MasterRTU/ModbusTCPMaster 메서드 확인
Read("__SKILL_DIR__/api/basis/communications-modbus.html")
```

> `__SKILL_DIR__` = 이 SKILL.md 파일이 위치한 디렉터리. Claude Code에서는 스킬 로드 시 자동으로 해석됨.
...
| `NU1605` / `Version conflict detected for Going.UI` | Going.UI(1.0.7.1)와 Going.UI.OpenTK 버전이 다름 | csproj에서 두 패키지 버전을 동일하게 통일 |
