...
namespace Going.Basis.Communications.Modbus.TCP
{
    ...
    public class MasterTCP
    {
        ...
        private void Modbus_BitWriteReceived(object? sender, ModbusTCPMaster.BitWriteEventArgs e)
        {
            if (e.Function == ModbusFunction.BITWRITE_F5)
                GetOrCreateDevice(e.Slave).Coils[e.StartAddress] = e.WriteValue;
        }

        private void Modbus_WordWriteReceived(object? sender, ModbusTCPMaster.WordWriteEventArgs e)
        {
            if (e.Function == ModbusFunction.WORDWRITE_F6)
                GetOrCreateDevice(e.Slave).HoldingRegister[e.StartAddress] = e.WriteValue;
        }

        private void Modbus_MultiBitWriteReceived(object? sender, ModbusTCPMaster.MultiBitWriteEventArgs e)
        {
        ...
        private void Modbus_MultiWordWriteReceived(object? sender, ModbusTCPMaster.MultiWordWriteEventArgs e)
        {
