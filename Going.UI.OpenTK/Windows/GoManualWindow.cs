...
{
    public class GoManualWindow : NativeWindow
    {
        #region Properties
        ...
        #region Constructor
        public GoManualWindow(int width, int height, WindowBorder Style)
        ...
        #region Invalidate
        public void Invalidate() => inv = true;
        #endregion
        ...
        public virtual unsafe void Run()
        {
            ...
            if (ds != null)
            {
                var lsT = Util.AllControls(ds.TitleBar);
                var lsL = Util.AllControls(ds.LeftSideBar);
                var lsR = Util.AllControls(ds.RightSideBar);
                var lsB = Util.AllControls(ds.Footer);
                List<IGoControl> ls = [.. lsT, .. lsL, .. lsR, .. lsB];
                foreach (var p in ds.Pages.Values) ls.AddRange(Util.AllControls(p));
                foreach (var w in ds.Windows.Values) ls.AddRange(Util.AllControls(w));
                foreach (var w in GoDialogs.SystemWindows.Values) ls.AddRange(Util.AllControls(w));

                foreach (var c in ls)
                    if (c is GoControl vc)
                        vc.SetView(false);

                ds.RequestInvalidate += () => Invalidate();
            }

            while (GLFW.WindowShouldClose(WindowPtr) == false)
            {
                var interval = Interval < 0 ? 0 : Interval;

                ProcessWindowEvents(IsEventDriven);

                if (inv || TKInputManager.Current.IsPlaying)
                {
                    NewInputFrame();
