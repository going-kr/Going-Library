...
- 렌더/레이아웃: 투명 컨테이너 — 자식(템플릿 트리)을 그대로 그림. **컨트롤 좌표는 Going 전체에서 부모 컨테이너 기준**이므로, 템플릿 컨트롤은 인스턴스 기준으로 자동 배치된다(기존 컨테이너 동작 그대로). 컴포넌트만의 레이아웃 로직 **없음**. 반응형이 필요하면 작성자가 기존 `Dock`(예: `Dock="Fill"`)/레이아웃 패널을 템플릿에 쓰면 된다 — 어디서나 동일.
...
1. **Param `type` 해석**: `string`/`int` 등 별칭 + 유저 VM 타입을 **로드된 어셈블리 simple-name 스캔**으로 찾을지(등록 0줄, 동명 시 첫 매치), 아니면 **명시 등록 API**를 둘지.
2. **미등록 대문자 태그 정책**: throw(기존 `Unknown Gudx tag`) vs 경고-후-스킵.

> ~~인스턴스 레이아웃~~ 결정 제거 — 컨트롤 좌표가 컨테이너 기준이라 인스턴스는 그냥 컨테이너이고, 반응형은 기존 `Dock`로 해결됨. 컴포넌트만의 레이아웃 이슈 없음.
